...
async function submit() {
  await userStore.register({email:form.value.email,password:form.value.password})
  ...
  <div class="flex items-center justify-center h-screen">
    <div class="flex flex-col max-w-md p-6 rounded-md sm:p-10 bg-gray-50 text-gray-800">
      ...
      <form @submit.prevent="submit">
        ...
        <div class="space-y-2">
          <div>
            <button type="submit" class="w-full px-8 py-3 font-semibold rounded-md bg-sky-600 text-gray-50">Sign Up</button>
          </div>
          <p class="px-6 text-sm text-center text-gray-600">Already have an account?
            <a rel="noopener noreferrer" href="/login" class="hover:underline text-sky-600">Sign In</a>.
          </p>
        </div>
      </form>
    </div>
